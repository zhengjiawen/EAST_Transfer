...
def resize_val_img(img, w, h, vertices):
    ...
    return img.resize((w, h), Image.BILINEAR), new_vertices
...
def extract_vertices(lines, data_flag='ic15'):
    ...
    if data_flag == 'ic15':
        ...
        for line in lines:
            parts = line.rstrip('\n').lstrip('\ufeff').split(' ')
            left, top, right, bottom = (int(x) for x in parts[:4])
            pos_list = [left, top, right, top, right, bottom, left, bottom]
...
class custom_dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        with open(self.gt_files[index], 'r') as f:
            lines = f.readlines()
        vertices, labels = extract_vertices(lines, 'ic15')
...
class IC13_dataset(data.Dataset):
    def __init__(self, img_path, gt_path, scale=0.25, length=512):
        super(IC13_dataset, self).__init__()
        self.img_files = [os.path.join(img_path, img_file) for img_file in sorted(os.listdir(img_path))]
        self.gt_files = [os.path.join(gt_path, gt_file) for gt_file in sorted(os.listdir(gt_path))]
        # print('img num:', len(self.img_files))
        # print('gt num:', len(self.gt_files))
    ...
    def __getitem__(self, index):
        ...
        #     print('index:', index)
        #     raise IndexError
        vertices, labels = extract_vertices(lines, 'ic13')
...
class valid_dataset(data.Dataset):
    def __init__(self, img_path, gt_path, scale=0.25, length=512, data_flag='ic15'):
    ...
    def __getitem__(self, index):
        ...
        img = Image.open(self.img_files[index]).convert('RGB')
        if self.data_flag != 'ic15':
            img, vertices = resize_val_img(img, 512,512, vertices)

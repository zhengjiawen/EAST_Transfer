...
from model import EAST #vgg16
from loss import Loss
from utils.logger_util import Logger as Logger
...
# tensorboardX writer
writer = SummaryWriter(global_args.fold)
# 1. set random seed
random.seed(2050)
np.random.seed(2050)
...
if not os.path.join(log_folder):
    os.mkdir(log_folder)
...
def train(source_train_loader, target_train_loader, model, criterion, loss_domain, optimizer, epoch):
    ...
    cls_total_loss = AverageMeter()
    geo_total_loss = AverageMeter()
    ...
    for i, (s_img, s_gt_score, s_gt_geo, s_ignored_map) in enumerate(source_train_loader):
        ...
        classify_loss, geo_loss = criterion(s_gt_score, pred_score, s_gt_geo, pred_geo, s_ignored_map)
        loss = classify_loss+geo_loss + loss_domain_s + loss_domain_t
        ...
        cls_total_loss.update(classify_loss.item(), s_img.size(0))
        geo_total_loss.update(geo_loss.item(), s_img.size(0))
        ...
        writer.add_scalar('iter/cls_loss', cls_total_loss.val, epoch*int(len(source_train_loader)+i))
        writer.add_scalar('iter/geo_loss', geo_total_loss.val, epoch*int(len(source_train_loader)+i))
        ...
        log.write('Epoch is [{}/{}], mini-batch is [{}/{}], time consumption is {:.8f}, batch_loss is {:.8f}'.format( \
            epoch + 1, global_args.epochs, i + 1, int(len(source_train_loader)), time.time() - start_time, loss.item()), is_terminal=0)
        log.write("\n",is_terminal=0)

    # output tensorboardX
    writer.add_scalar('epoch/total_loss', epoch_loss.avg, epoch)
    writer.add_scalar('epoch/cls_loss', cls_total_loss.avg, epoch)
    writer.add_scalar('epoch/geo_loss', geo_total_loss.avg, epoch)
...
def eval(model, val_dataloader, criterion, loss_domain, epoch):
    ...
    with torch.no_grad():
        for i, (img, gt_score, gt_geo, ignored_map) in enumerate(val_dataloader):
            ...
            classify_loss, geo_loss = criterion(gt_score, pred_score, gt_geo, pred_geo, ignored_map)

            # domain label
            domain_t = Variable(torch.ones(pred_cls.size(0)).long().cuda())
            loss_domain_t = loss_domain(pred_cls, domain_t)

            detection_loss = classify_loss+geo_loss

            loss = detection_loss+loss_domain_t

            # record loss
            epoch_loss.update(loss.item(), img.size(0))
            detection_total_loss.update(detection_loss.item(), img.size(0))
            cls_total_loss.update(classify_loss.item(), img.size(0))
            geo_total_loss.update(geo_loss.item(), img.size(0))
    ...
    writer.add_scalar('eval/cls_loss', cls_total_loss.avg, epoch)
    writer.add_scalar('eval/geo_loss', geo_total_loss.avg, epoch)
    ...
    log.write( 'Epoch[{}], Eval, epoch_loss is {:.8f}, epoch_time is {:.8f}'.format(epoch, epoch_loss.avg, time.time() - epoch_time))
    log.write('\n')
...
def main():
    source_train_set = custom_dataset(global_args.train_data_path, global_args.train_gt_path)
    target_train_set = custom_dataset(global_args.target_data_path, global_args.target_gt_path)
    valid_train_set = valid_dataset(global_args.val_data_path, global_args.val_gt_path)

    source_train_loader = data.DataLoader(source_train_set, batch_size=global_args.batch_size,
                                          shuffle=True, num_workers=global_args.num_workers, drop_last=True)
    target_train_loader = data.DataLoader(target_train_set, batch_size=global_args.batch_size,
                                          shuffle=True, num_workers=global_args.num_workers, drop_last=True)
    valid_loader = data.DataLoader(valid_train_set, batch_size=global_args.batch_size,
                                         shuffle=False, num_workers=global_args.num_workers, drop_last=False)
    ...
    model = EAST()
    if os.path.exists(global_args.pretrain_model_path):
        model.load_state_dict(torch.load(global_args.pretrain_model_path))
    ...
    total_epoch = global_args.epochs
    optimizer = torch.optim.Adam(model.parameters(), lr=global_args.lr)
    scheduler = lr_scheduler.MultiStepLR(optimizer, milestones=[total_epoch // 3, total_epoch * 2 // 3], gamma=0.1)

    # resume
    if os.path.exists(global_args.resume):
        checkpoint = torch.load(global_args.pretrain_model_path)
        model.load_state_dict(checkpoint['state_dict'])
        best_loss = checkpoint['beat_loss']
    ...
    for epoch in range(current_epoch_num, total_epoch):
        ...
        if (epoch + 1) % int(global_args.save_interval) == 0 :
